...
void Ext2Properties::set_properties(Ext2File *file)
{
    ...
    properties->typeval->setText(QString::fromLocal8Bit(get_type_string(file->file_type)));
    ...
    if(file->file_size < 1024)
    {
        ...
        sizestr = QString::fromLocal8Bit("%1 Bytes").arg(file->file_size);
        szstrblock = QString::fromLocal8Bit("%1 Bytes").arg(sizeblock);

    }
    else if ((file->file_size >= 1024) && (file->file_size < (1024 * 1024)))
    {
        sizestr = QString::fromLocal8Bit("%1 KB (%L2 Bytes)").arg((double)file->file_size/1024, 0, 'g', 3).arg(file->file_size);
        szstrblock = QString::fromLocal8Bit("%1 KB (%L2 Bytes)").arg((double)sizeblock/1024, 0, 'g', 3).arg(sizeblock);
    }
    else if ((file->file_size >= (1024 * 1024)) && (file->file_size < (1024 * 1024 * 1024)))
    {
        sizestr = QString::fromLocal8Bit("%1 MB (%L2 Bytes)").arg((double)file->file_size/(1024 * 1024), 0, 'g', 3).arg(file->file_size);
        szstrblock = QString::fromLocal8Bit("%1 MB (%L2 Bytes)").arg((double)sizeblock/(1024 * 1024), 0, 'g', 3).arg(sizeblock);
    }
    else if (file->file_size >= (1024 * 1024 *1024))
    {
        sizestr = QString::fromLocal8Bit("%1 GB (%L2 Bytes)").arg((double)file->file_size/(1024 * 1024 * 1024), 0, 'g', 3).arg(file->file_size);
        szstrblock = QString::fromLocal8Bit("%1 GB (%L2 Bytes)").arg((double)sizeblock/(1024 * 1024 * 1024), 0, 'g', 3).arg(sizeblock);
    }
