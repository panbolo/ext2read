...
int LVM::scan_pv()
{
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        LOG("\n%s", metadata);
        pv_metadata = QString::fromLocal8Bit(metadata, label->pv_length);
        parse_metadata();
        delete [] metadata;
        break;
    }
...
void VolumeGroup::logical_mount()
{
    ...
    for(i = lvolumes.begin(); i != lvolumes.end(); i++)
    {
        ...
        LOG("PE start %d offset %d extoff %d B=%d A=%d %s\n", root->pvolumes->pe_start,
            root->pvolumes->offset, lvol->this_group->extent_size, root->start_extent, root->stripe->stripe_start_extent, lvol->volname.toLocal8Bit().data());
